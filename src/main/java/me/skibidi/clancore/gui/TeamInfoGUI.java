...
public class TeamInfoGUI {
    ...
    public static void open(Player player, Team team, int page) {
        ...
        Inventory inv = Bukkit.createInventory(null, 54, "§bTeam Info §7(Page " + (page + 1) + "/" + Math.max(1, totalPages) + ")");

        // Info item ở slot 4
        ItemStack infoItem = new ItemStack(Material.BOOK);
        ItemMeta infoMeta = infoItem.getItemMeta();
        if (infoMeta != null) {
            infoMeta.setDisplayName("§bTeam Information");
            List<String> lore = new ArrayList<>();
            lore.add("§7Members: §b" + members.size() + "/" + team.getMaxSize());
            lore.add("§7Leader: §b" + Bukkit.getOfflinePlayer(team.getLeader()).getName());
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            if (skullMeta != null) {
                ...
                if (team.getLeader().equals(memberUuid)) {
                    lore.add("§6§lLeader");
                }
